...
contract ConverterRamp is Ownable {
    ...
    event RequiredRebuy(Token token, uint256 amount);
    event Return(Token token, address to, uint256 amount);
    event OptimalSell(Token token, uint256 amount);
    ...
    function pay(
        ...
        Token _fromToken,
        bytes32[4] calldata _loanParams,
        bytes calldata _oracleData,
        uint256[3] calldata _convertRules
    ) external payable returns (bool) {
        Token rcn = Engine(address(uint256(_loanParams[I_ENGINE]))).rcn();
        ...
        require(
            _rebuyAndReturn({
                ...
                _fromToken: rcn,
                _toToken: _fromToken,
                _amount: rcn.balanceOf(address(this)) - initialBalance,
    ...
    function requiredLendSell(
        TokenConverter _converter,
        Token _fromToken,
        bytes32[3] calldata _loanParams,
        bytes calldata _oracleData,
        ...
        return _getOptimalSell(
            _converter,
            _fromToken,
            Engine(address(uint256(_loanParams[I_ENGINE]))).rcn(),
    ...
    function lend(
        ...
        // Lend loan
        require(rcn.approve(address(uint256(_loanParams[I_MARGIN_SPEND])), bought), "Error approving lend token transfer");
        require(_executeLend(_loanParams, _oracleData, _cosignerData), "Error lending the loan");
        require(rcn.approve(address(uint256(_loanParams[I_MARGIN_SPEND])), 0), "Error removing approve");
    ...
    function _rebuyAndReturn(
        ...
        require(_spentAmount.sub(bought) <= maxSpend || maxSpend == 0, "Max spend exceeded");
    ...
    function _getOptimalSell(
        ...
        if (_extraSell == AUTO_MARGIN) {
            ...
            return _applyRate(_requiredTo, sellRate).mult(uint256(100000).add(_extraSell)) / 100000;
        }
    }
    ...
    function _convertSafe(
        ...
        if (_fromToken != ETH_ADDRESS) require(_fromToken.approve(address(_converter), _amount), "Error approving token transfer");
        uint256 prevBalance = _toToken != ETH_ADDRESS ? _toToken.balanceOf(address(this)) : address(this).balance;
        ...
        require(
            boughtAmount == (_toToken != ETH_ADDRESS ? _toToken.balanceOf(address(this)) : address(this).balance) - prevBalance,
            "Bought amound does does not match"
        );
        if (_fromToken != ETH_ADDRESS) require(_fromToken.approve(address(_converter), 0), "Error removing token approve");
        return boughtAmount;
    }
    ...
    function _executeOptimalPay(
        ...
        Engine engine = Engine(address(uint256(_params[I_ENGINE])));
        ...
        require(rcn.approve(address(engine), _rcnToPay), "Error on payment approve");
        require(engine.pay(index, toPay, address(uint256(_params[I_PAY_FROM])), _oracleData), "Error paying the loan");
        require(rcn.approve(address(engine), 0), "Error removing the payment approve");
    ...
    function _executeLend(
        ...
        Engine engine = Engine(address(uint256(_params[I_ENGINE])));
        uint256 index = uint256(_params[I_INDEX]);
        return engine.lend(index, _oracleData, Cosigner(address(uint256(_params[I_LEND_COSIGNER]))), _cosignerData);
    }
    ...
    function _executeTransfer(
        ...
        return Engine(address(uint256(_params[I_ENGINE]))).transfer(_to, uint256(_params[1]));
    }
    ...
    function _applyRate(
        ...
        return _amount.mult(_rate) / 10 ** 18;
    }
    ...
    function _getRequiredRcnLend(
        ...
        Engine engine = Engine(address(uint256(_params[I_ENGINE])));
        uint256 index = uint256(_params[I_INDEX]);
        Cosigner cosigner = Cosigner(address(uint256(_params[I_LEND_COSIGNER])));

        if (cosigner != Cosigner(0)) {
            required += cosigner.cost(address(engine), index, _cosignerData, _oracleData);
        }
