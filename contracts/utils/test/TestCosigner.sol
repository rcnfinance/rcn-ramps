...
contract TestCosigner is Cosigner, BytesUtils {
    ...
    function buildData(bytes32 a, bytes32 b) internal pure returns (bytes o) {
        assembly {
            let size := 64
            o := mload(0x40)
    ...
    function cost(address, uint256, bytes _data, bytes) constant public returns (uint256) {
        return uint256(readBytes32(_data, 1));
    }

    function requestCosign(Engine engine, uint256 index, bytes _data, bytes) public returns (bool) {
        if(readBytes32(_data, 0) == keccak256("test_oracle")) {
            require(engine.cosign(index, uint256(readBytes32(_data, 1))));
            return true;
        } else {
            return true;
        }
